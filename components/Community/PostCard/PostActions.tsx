...
import { View, Text, TouchableOpacity, Share, Animated, StyleSheet } from 'react-native';
...
export default function PostActions({ post, onCommentPress }: Props) {
  ...
  const handleLike = async () => {
    ...
    Animated.sequence([
      ...
      Animated.spring(heartScale, { toValue: 1.0, useNativeDriver: true, speed: 50 }),
    ]).start();
  ...
  return (
    <View style={styles.container}>
      <View style={styles.leftGroup}>
        ...
        <TouchableOpacity onPress={handleLike} style={styles.action} activeOpacity={0.7}>
          <Animated.View style={{ transform: [{ scale: heartScale }] }}>
            <Heart size={22} color={liked ? likedColor : iconColor} fill={liked ? likedColor : 'none'} />
          </Animated.View>
          <Text style={[styles.count, { color: liked ? likedColor : Colors.text.tertiary }]}>
            {formatCount(likeCount)}
          </Text>
        </TouchableOpacity>

        {/* Comment */}
        <TouchableOpacity onPress={onCommentPress} style={styles.action} activeOpacity={0.7}>
          <MessageCircle size={22} color={iconColor} />
          <Text style={styles.count}>{formatCount(post.comment_count)}</Text>
        </TouchableOpacity>

        {/* Share */}
        <TouchableOpacity onPress={handleShare} style={styles.action} activeOpacity={0.7}>
      ...
      {/* Bookmark */}
      <TouchableOpacity onPress={() => setBookmarked((b) => !b)} activeOpacity={0.7}>
        <Bookmark
          size={22}
